...
if not os.path.exists(SUB_CODE_REFR_PATH):
    with open(SUB_CODE_REFR_PATH, "w") as fp1:
        with open("subject_code_refer.json", "r") as fp2:
            fp1.write(fp2.read())
if not os.path.exists(CONFIG_FILE_PATH):
    with open(CONFIG_FILE_PATH, "w") as fp1:
        with open("configs.json", "r") as fp2:
            fp1.write(fp2.read())

subject_by_code = json.load(open(SUB_CODE_REFR_PATH, "r"))
configs_ = json.load(open(CONFIG_FILE_PATH, "r"))
...
def initDataFrame(dataLinePairs):
    ...
    columns_ = []
    list(map(lambda sub_code: columns_.extend([f"MARKS"]), unique_sub_codes))
    df_ = DataFrame(
        columns=["roll_no", "student_name", "gender"] +
                columns_ +
                ["Max Marks OA", "Total OA", "Percentage OA", "Average OA"] +
                ["Max Marks B5", "Total B5", "Percentage B5", "Average B5"] +
                ["result", "compartment"]
    )
...
def writeToExcel():
    ...
    for uni_subcode in unique_sub_codes:
        df_single = DataFrame(df_over_all[[0, 1, 2, df_col_idxr]])
        ...
        df_single.columns = ["Roll No.", "Name", "Gender", "Marks Obtained"]
        df_single = df_single.dropna(subset=['Marks Obtained'], how='all')
...
def on_configs():
    ...
    def on_add_sub():
        ...
        all_subject_by_codes = json.load(open(SUB_CODE_REFR_PATH, "r"))
        ...
        json.dump(all_subject_by_codes, open(SUB_CODE_REFR_PATH, "w"))
