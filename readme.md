## Result Analyzer Repository
It is a series of software belonging to result analyzer family which helps in building, cleaning, processing and analyzing of CBSE board result data.  
It isn't a single software but a series of software due to CBSE result text file pattern changes over time each version is specialized for one type of file. It ensures that if user have a file of very old pattern so they can still process it using appropiate version of this series.

## Function of these softwares
These software is personally build for sort and structuring data of CBSE board examinations result of students. All of them have nearly same functions and options but internal working and logics are different.  
All requires a txt file which contain data in a format describe below then these software will sort students, their gender, their roll number and percentage average etc. Sorted data can be exported as an Excel or JSON file.

**NOTE:** Format can slightly very among different versions, consider reading readme of specific version.
...
# Note For Devloper
Naming conventions of project will be:-  

### Release tag naming convention
RAC`<CLASS>`-`<SHORT SPECIFICATION>`-V`<VERSION>`
- CLASS: like 10 12 etc.
- SHORT SPECIFICATION: NG for no grade, WG for with grade and any other short form repersenting that software's usecase.
- VERSION: 1.x.x or 2.x.x or 3.x.x .... starting from 1.0.0

### Release title naming convention
RA-C`<CLASS>`-`<FULL SPECIFICATION>`-v`<VERSION>`
- CLASS: like 10 12 etc.
- FULL SPECIFICATION: like No Grade, With Grade and any other short form repersenting that software's usecase.
- VERSION: 1.x.x or 2.x.x or 3.x.x .... starting from 1.0.0

### Installer naming convention
ResultAnalyzer-C`<CLASS>`.`<FULL SPECIFICATION>`.setup.exe
- CLASS: like 10 12 etc.
